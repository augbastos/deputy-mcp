"""Dual-format (markdown / JSON) rendering core — fastmcp-free and shared.

This neutral leaf module holds the single set of model -> markdown / JSON
renderers used by BOTH the MCP server layer (via
:mod:`deputy_mcp.server.formatting`, a thin re-export) and the CLI. Keeping it
free of any ``fastmcp`` import means the client-only CLI can render exactly like
the server without pulling in the server's dependencies, so a shift is shown with
the same wall-clock time on every surface (no UTC-vs-company-timezone fork).
...
A caller that has no company record (e.g. the CLI, which does not make an extra
API call to fetch one) passes ``None`` and renders in UTC through this same code
path — parameterised by ``tz``, not a second renderer.
"""
...
def resolve_timezone(company: Company | None) -> tuple[tzinfo, str]:
    ...
    label preserves the intended zone so the output stays honest. Passing ``None``
    (no company record available) yields ``(UTC, "UTC")`` — the honest default for
    a caller that cannot resolve a company timezone.
    """
